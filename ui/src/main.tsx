...
function App() {
  ...
  const [job, setJob] = useState<Job | null>(() => { const id = new URLSearchParams(location.search).get("drawing"); return id && /^[a-f0-9]{32}$/.test(id) ? { id, filename: "Saved drawing", status: "queued", message: "Loading saved drawing", model_available: false, download_available: false } : null; });
  ...
  const previewUrl = job && job.status !== 'queued' ? `/api/drawings/${job.id}/files/drawing` : '';
  ...
  async function generate() {
    if (!file || busy) return;
    generation.current++; setPosting(true); setError(''); setMesh(null); setJob(null);
  ...
  const phases = [{ key: 'Upload', done: !!job }, { key: 'Read drawing', done: !!job && ['building', 'ready', 'review'].includes(job.status) }, { key: 'Build & inspect', done: job?.status === 'ready' }, { key: 'Download', done: false }];
      ...
      <main>
        ...
        <div className="work-grid">
          <section className="input-panel" aria-label="Drawing upload and settings">
            ...
            <button className="primary-button" onClick={generate} disabled={!file || busy}>{busy ? <><LoaderCircle size={17} className="spin" />{job?.message || 'Uploading drawing…'}</> : <><span>{job ? 'Generate again' : 'Generate 3D preview'}</span><ArrowUpRight size={19} /></>}</button>
          ...
          <section className="model-panel" aria-label="Model preview and downloads">
            ...
            {job && terminal.has(job.status) && <div className="review-card"><button onClick={() => setDetails(!details)} aria-expanded={details}><span><FileText size={17} />{job.status === 'review' ? 'Profile needs review' : 'Drawing notes & feature coverage'}</span><Plus size={17} className={details ? 'rotate-plus' : ''} /></button><p>{mesh ? "Only the axial body is built. Holes, slots, pins and ports need further modeling and review." : "No model was generated. Review the reading notes and profile before creating geometry."}</p>{details && <div className="review-details">{job.unsupported_features?.length ? <><strong>Features outside this preview</strong><ul>{job.unsupported_features.map((text, i) => <li key={i}>{text}</li>)}</ul></> : null}{job.warnings?.length ? <><strong>Reading notes</strong><ul>{job.warnings.map((text, i) => <li key={i}>{text}</li>)}</ul></> : null}<strong>Proposed callouts · {job.requirements?.length || 0}</strong><div className="ledger-table">{job.requirements?.map(r => <div key={r.id}><span className="mono">{r.id}</span><span>{r.raw_text}</span><small>{r.resolved_unit}</small></div>)}</div></div>}</div>}
          </section>
        </div>
