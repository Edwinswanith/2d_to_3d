...
function App() {
  ...
  const [legacyRuntime, setLegacyRuntime] = useState(false);
  useEffect(() => {
    const controller = new AbortController();
    fetch('/api/health', {signal:controller.signal}).then(r => r.json()).then(info => setLegacyRuntime(info.mode === 'legacy-faceted-body-workspace')).catch(() => {});
  ...
  const previewUrl = job && ['context', 'inventory', 'reading', 'checking', 'building', 'previewing', 'ready', 'review'].includes(job.status) ? `/api/drawings/${job.id}/files/drawing` : '';
  ...
  function choose(next?: File) {
    ...
    history.replaceState(null, "", location.pathname); generation.current++; setFile(next); setJob(null); setMesh(null); setError(''); setPreviewUnavailable(false);
  }

  useEffect(() => {
    if (!job || job.status === 'failed' || (terminal.has(job.status) && (!job.model_available || mesh || previewUnavailable))) return;
    const id = job.id;
    ...
    async function poll() {
      try {
        ...
        if (terminal.has(next.status) && next.model_available) {
          try {
            ...
            if (meshResponse.ok) {
              ...
              if (version !== generation.current || stopped) return;
              setMesh(geometry);
            } else if ([404, 409, 425].includes(meshResponse.status) && ++meshAttempts < 10) {
              waitingForMesh = true;
            } else {
              throw new Error('Preview unavailable');
            }
          } catch(e) {
            if (controller.signal.aborted || version !== generation.current || stopped) return;
            setError('The model preview could not be loaded. Available downloads remain below.');
  ...
  }, [job?.id, job?.status, mesh, previewUnavailable]);

  async function generate() {
    if (!file || busy) return;
    generation.current++; startedAt.current = Date.now(); setPosting(true); setError(''); setMesh(null); setJob(null); setPreviewUnavailable(false);
  ...
  const legacyModel = !!job && !job.revision_b;
  const phases = [{ key: 'Upload', done: !!job }, { key: 'Read drawing', done: !!job && ['checking', 'building', 'previewing', 'ready', 'review'].includes(job.status) }, { key: 'Build & inspect', done: !!job?.model_available }, { key: 'Download', done: canDownloadCad || canDownloadReport }];
  return <div className="app-shell">
    ...
    <div className="workspace">
      ...
      <main>
        <div className="page-heading"><div><p className="eyebrow">DRAWING TO 3D / 01</p><h1>Make room for another dimension.</h1><p>Read a gland-ring drawing and account for its features before building.</p></div><span className="version-badge mono">{legacyRuntime ? 'LEGACY BODY DRAFT' : 'FEATURE DRAFT · REV B'}</span></div>
        <div className="work-grid">
          <section className="input-panel" aria-label="Drawing upload and settings">
            ...
            <div className="select-wrap"><select id="units" value={units} onChange={e => setUnits(e.target.value as UnitChoice)} disabled={busy}><option value="auto">{legacyRuntime ? 'Auto detect · default mm' : 'Read units from drawing'}</option><option value="mm">Millimetres (mm)</option><option value="cm">Centimetres (cm)</option><option value="in">Inches (in)</option></select><ChevronDown size={15} /></div>
            <p className="field-help">{legacyRuntime ? 'Legacy body mode defaults to mm when the drawing omits units. This runtime does not run Revision B completeness checks.' : 'Revision B requires explicit units. Missing or conflicting units stop for review. If the title block contradicts its own printed envelope (for example inches beside Ø218.0), select the real unit here: it is recorded as an engineer unit correction and stays visible as a finding. A plausible sheet is never overridden.'}</p>
            ...
            <button className="primary-button" onClick={generate} disabled={!file || busy}>{busy ? <><LoaderCircle size={17} className="spin" />{stageFor(progressStatus).title}…</> : <><span>{job ? 'Generate again' : 'Generate 3D draft'}</span><ArrowUpRight size={19} /></>}</button>
            <p className="cloud-note">Auditing sends this drawing to Gemini and, when configured, Document AI. The API key stays on the server.</p>
          </section>
          <section className="model-panel" aria-label="Model preview and downloads">
            {job?.completion === "LEGACY_UNVERIFIED" && <p className="legacy-warning">Historical partial body. Holes, ports and other features may be missing. Revision B completeness checks have not run.</p>}
            {<Viewer mesh={mesh} legacy={legacyModel} selectedFeature={selectedFeature} overlay={busy ? <GenerationProgress status={progressStatus} startedAt={startedAt.current} drawingUrl={job ? `/api/drawings/${job.id}/files/drawing` : undefined} /> : null} />}

            {<div className="model-caption"><div><span className="eyebrow">{mesh ? 'YOUR MODEL' : 'THE NEXT DIMENSION'}</span><h2>{mesh ? job?.part_name || 'Gland ring draft' : 'A better perspective starts here.'}</h2><p>{mesh ? `${job?.drawing_number || file?.name} · Source units ${job?.unit?.toUpperCase()} · ${job?.unit_source}` : 'Orbit the solid, inspect its form, and export a STEP file for future CAD work.'}</p></div><div className="caption-icon"><Box size={24} /></div></div>}
            {mesh && <div className="result-strip"><span><Check size={15} />{legacyModel ? 'Legacy draft · geometry unverified' : 'STEP integrity passed'}</span><span className="mono">{job?.measurements?.bbox.map(x => x.toFixed(2)).join(' × ')} MM</span></div>}
            <div className="download-bar"><div><ArrowDownToLine size={19} /><div><strong>{canDownloadCad ? legacyModel ? 'Keep your body draft' : 'Keep your feature draft' : canDownloadReport ? 'Review package ready' : 'Your design, ready to keep'}</strong><p>{canDownloadCad ? 'STEP draft · assumptions and drawing conformance need review' : canDownloadReport ? 'Review the findings before creating geometry.' : 'STEP for CAD · STL for mesh workflows'}</p></div></div><div className="download-actions">{canDownloadCad ? <><a className="download-step" href={`/api/drawings/${job!.id}/files/step`} download>STEP <ArrowDownToLine size={15} /></a><a className="download-secondary" href={`/api/drawings/${job!.id}/files/stl`} download>STL</a><a className="download-secondary" href={`/api/drawings/${job!.id}/files/report`} download>Report</a></> : canDownloadReport ? <><a className="download-step" href={`/api/drawings/${job!.id}/files/report`} download>Report <ArrowDownToLine size={15} /></a><button className="download-secondary" disabled>STEP</button><button className="download-secondary" disabled>STL</button></> : <><button className="download-step" disabled>STEP <ArrowDownToLine size={15} /></button><button className="download-secondary" disabled>STL</button></>}</div></div>
            {job?.revision_b && job.status === 'review' && <><ModelReview job={job} onSelectFeature={setSelectedFeature} onBuild={() => { setMesh(null); setError(''); setPreviewUnavailable(false); setJob(prev => prev ? {...prev, status: 'building'} : prev); startedAt.current = Date.now(); }} /><RevisionBReview key={job.id} result={job} /></>}
            {job && !job.revision_b && terminal.has(job.status) && <div className="review-card"><button onClick={() => setDetails(!details)} aria-expanded={details}><span><FileText size={17} />{job.status === 'review' ? 'Profile needs review' : 'Drawing notes & feature coverage'}</span><Plus size={17} className={details ? 'rotate-plus' : ''} /></button><p>{mesh ? "Only the axial body is built. Holes, slots, pins and ports need further modeling and review." : job.status === "review" ? job.message : "No model was generated. Review the reading notes and profile before creating geometry."}</p>{details && <div className="review-details">{job.unsupported_features?.length ? <><strong>Features outside this preview</strong><ul>{job.unsupported_features.map((text, i) => <li key={i}>{text}</li>)}</ul></> : null}{job.warnings?.length ? <><strong>Reading notes</strong><ul>{job.warnings.map((text, i) => <li key={i}>{text}</li>)}</ul></> : null}<strong>Proposed callouts · {job.requirements?.length || 0}</strong><div className="ledger-table">{job.requirements?.map(r => <div key={r.id}><span className="mono">{r.id}</span><span>{r.raw_text}</span><small>{r.resolved_unit}</small></div>)}</div></div>}</div>}
          </section>
        </div>
